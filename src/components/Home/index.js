...
class Home extends Component {
  ...
  componentDidMount = () => {
    this.intervalId = setInterval(this.gameTimerUpdate, 1000)
  }
  ...
  clickActiveTab = activeTabId => {
    this.setState({activeTabId})
  }

  onClickImageMatch = thumbnailUrl => {
    const {randomImageUrl} = this.state

    const clickedImageList = imagesList.filter(
      eachItem => eachItem.thumbnailUrl === thumbnailUrl,
    )
    const {imageUrl} = clickedImageList[0]
  ...
  renderMatchGameDetails = () => {
    const {activeTabId, randomImageUrl} = this.state

    const activeTabList = this.renderActiveTabList()
    ...
    return (
      <div className="match-game-container">
        ...
        <ul className="active-tab-List-container">
          {activeTabList.map(eachList => (
            <ImageCard
              imageItem={eachList}
              key={eachList.id}
              onClickImageMatch={this.onClickImageMatch}
            />
          ))}
        </ul>
      </div>
  ...
  renderScoreCardDetails = () => {
    ...
    return (
      <div className="score-card-container">
        ...
        />
        <p className="game-score-label">Your Score</p>
        <p className="game-score-value">{score}</p>
        <button
          type="button"
  ...
  render() {
    const {score, gameTimer, isGameOver} = this.state

    return (
      <div className="app-container">
        <NavBar score={score} gameTimer={gameTimer} />:
        {isGameOver
          ? this.renderScoreCardDetails()
          : this.renderMatchGameDetails()}
      </div>
    )
  }
}
